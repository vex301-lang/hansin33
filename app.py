...
def generate_step_story(title, idx):
    ...
    prompt = (
        f"주인공 정보: {character}\n\n"
        f"지금까지의 이야기 (참고만 하세요):\n{prev}\n\n"
        f"'{title}'로 시작하는 새로운 장면을 한 단락(200~300자)만 써 주세요.\n"
        "결말을 쓰지 말고, 다음에 이어질 이야기를 남겨 두세요.\n"
        "초등학교 3학년이 이해하기 쉬운 부드러운 문체로 작성해 주세요."
    )
    
    try:
        with st.spinner(f"‘{title}’ 장면을 만드는 중이에요..."):
            resp = client.responses.create(model="gpt-4o-mini", input=prompt, max_output_tokens=500)
...
if any(st.session_state[f"story_{i}"].strip() for i in range(8)):
    ...
    st.write(story_text)
    file_name = f"{cls}_{num}_{name}_story.txt".replace(" ", "_") or "my_story.txt"
    st.download_button("📥 이야기 저장하기 (txt)", data=story_text, file_name=file_name, mime="text/plain")
