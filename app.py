...
if any(st.session_state[f"story_{i}"].strip() for i in range(8)):
    ...
    link_col2.markdown(
        "<a href='#' onclick='navigator.clipboard.writeText(document.body.innerText);' "
        "style='text-decoration:none; background-color:#2196F3; color:white; "
        "padding:10px 20px; border-radius:8px; display:inline-block;'>📋 이야기 복사하기</a>",
        unsafe_allow_html=True
    )
